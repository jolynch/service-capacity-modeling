...
def _es_io_per_read(node_size_gib, segment_size_mb=512):
    ...
    segments = max(1, gb // segment_size_mb)
...
def _estimate_elasticsearch_cluster_zonal(
    ...
    cluster = compute_stateful_zone(
        ...
        # Elasticsearch requires ephemeral disks to be % full because leveled
        # compaction can make progress as long as there is some headroom
        required_disk_space=lambda x: x * 1.25,
...
class NflxElasticsearchCapacityModel(CapacityModel):
    ...
    def capacity_plan(
        ...
        max_local_disk_gib: int = extra_model_arguments.get("max_local_disk_gib", 4096)
    ...
    def default_desires(user_desires, extra_model_arguments: Dict[str, Any]):
        acceptable_consistency = set(
            (
                ...
                # This will make writes very expensive due to refresh and
                # waitfor
        ...
        # Penalize refresh and waitfor writes
        if (
            user_desires.query_pattern.access_consistency
            == AccessConsistency.read_your_writes
        ):
            rf_write_latency.low = rf_write_latency.low * 2
            rf_write_latency.mid = rf_write_latency.mid * 2
            rf_write_latency.high = rf_write_latency.high * 2
